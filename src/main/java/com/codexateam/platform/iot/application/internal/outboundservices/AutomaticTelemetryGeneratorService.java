...
public class AutomaticTelemetryGeneratorService {
    ...
    /**
     * Genera telemetría para todos los bookings activos cada 5 segundos.
     * Ciclo infinito mientras existan bookings activos.
     */
    ...
    public void generateTelemetryForActiveTrips() {
        ...
        List<Booking> activeBookings = bookingRepository.findByBookingStatus_StatusAndStartDateLessThanEqualAndEndDateGreaterThanEqual(
                BOOKING_STATUS_CONFIRMED, now, now);

        if (activeBookings.isEmpty()) {
            logger.debug("No active confirmed bookings in current window");
            return;
        }
        ...
        for (Booking booking : activeBookings) {
            ...
            try {
                // Initialize defaults
                vehicleFuelLevels.putIfAbsent(vehicleId, 100.0);
                vehicleStates.putIfAbsent(vehicleId, VehicleState.DETENIDO);

                // Load last telemetry for start position
                Telemetry latest = telemetryRepository
                        .findFirstByVehicleIdOrderByCreatedAtDesc(vehicleId)
                        .orElse(null);

                // Decide action by state
                ...
                if (state == VehicleState.DETENIDO) {
                    Long waitUntil = vehicleWaitUntil.get(vehicleId);
                    if (waitUntil == null) {
                        // Just arrived or no route yet -> start waiting window
                        vehicleWaitUntil.put(vehicleId, now.getTime() + STOP_WAIT_MILLIS);
                        // Ensure we store a stopped telemetry point (speed 0) at current location
                        double[] current = latest != null
                                ? new double[]{latest.getLatitude(), latest.getLongitude()}
                                : getRandomCoordinateInLima();
                        saveOrUpdateTelemetry(vehicleId, current[0], current[1], 0.0, vehicleFuelLevels.get(vehicleId));
                        logger.info("Vehículo {} DETENIDO por 10s", vehicleId);
                        continue;
                    }
                    if (now.getTime() >= waitUntil) {
                        // REINICIO: plan new route from current position to random point
                        double[] startPoint = latest != null
                                ? new double[]{latest.getLatitude(), latest.getLongitude()}
                                : getRandomCoordinateInLima();
                        ...
                        vehicleStates.put(vehicleId, VehicleState.MOVIENDO);
                        vehicleWaitUntil.remove(vehicleId);
                        logger.info("Vehículo {} REINICIO: nueva ruta con {} puntos", vehicleId, newQueue.size());
                    } else {
                        // Still waiting, keep speed 0 and no fuel consumption
                        double[] current = latest != null
                                ? new double[]{latest.getLatitude(), latest.getLongitude()}
                                : getRandomCoordinateInLima();
                        saveOrUpdateTelemetry(vehicleId, current[0], current[1], 0.0, vehicleFuelLevels.get(vehicleId));
                ...
                // MOVIENDO
                routeQueue = vehicleRoutes.get(vehicleId);
                if (routeQueue == null || routeQueue.isEmpty()) {
                    // No planned route -> plan from current to random
                    double[] startPoint = latest != null
                            ? new double[]{latest.getLatitude(), latest.getLongitude()}
                            : getRandomCoordinateInLima();
                    double[] endPoint = getRandomCoordinateInLima();
                    ...
                    vehicleStates.put(vehicleId, VehicleState.MOVIENDO);
                    routeQueue = newQueue;
                    logger.info("Vehículo {} planificó ruta con {} puntos", vehicleId, newQueue.size());
                }
                ...
                if (next != null) {
                    ...
                    double fuel = consumeFuel(vehicleId); // consume only when moving
                    saveOrUpdateTelemetry(vehicleId, next[0], next[1], speed, fuel);
                    logger.debug("Vehículo {} MOVIENDO lat={}, lng={} speed={} fuel={} restantes={}",
                            vehicleId,
                ...
                if (routeQueue.isEmpty()) {
                    // LLEGADA -> DETENIDO y esperar 10s
                    vehicleStates.put(vehicleId, VehicleState.DETENIDO);
                    vehicleWaitUntil.put(vehicleId, now.getTime() + STOP_WAIT_MILLIS);
                    // Persist last point with speed=0
                    Telemetry last = telemetryRepository
                            .findFirstByVehicleIdOrderByCreatedAtDesc(vehicleId)
                            .orElse(null);
                    if (last != null) {
                        saveOrUpdateTelemetry(vehicleId, last.getLatitude(), last.getLongitude(), 0.0, vehicleFuelLevels.get(vehicleId));
                    }
                    logger.info("Vehículo {} LLEGADA: entra en DETENIDO por 10s", vehicleId);
                }

            } catch (Exception e) {
                logger.error("Error in autonomous simulation for vehicle {}", vehicleId, e);
            }
        }
    }
